...
AUTO_MEMORY_SYSTEM_HINT = """\
An automatic memory filter is enabled. When the user asks you to remember, save, forget, or update something, acknowledge the request naturally; do not say that you cannot store memories. The filter will handle memory changes after your response.
"""
...
11. Each Memory should be short, no more that 75 words.
12. Memory should **reflect a fact** and be in present simple. It should not be in present continious, since present continious often describes temporary state.
</key_instructions>
...
class Filter:
    ...
    async def auto_memory(
        ...
        related_memories = await self.get_related_memories(
            messages=messages, user=user, emitter=emitter
        )
        ...
        try:
            ...
            for action in action_plan.actions:
                if action.action == "update" or action.action == "delete":
                    for mem in related_memories:
                        if mem.mem_id == action.id:
                            action.content = mem.content
                            break
    ...
    async def apply_memory_actions(
        ...
        operations = {
            "delete": {
                ...
                "status_msg": lambda a: f"Deleted memory: {a.content}",
            },
            "update": {
                ...
                "status_msg": lambda a: f"Updated memory: {a.content} => {a.new_content}",
            },
            "add": {
                ...
                "status_msg": lambda a: f"Saved memory: {a.content}",
            },
        }
        ...
        if self.user_valves.show_status:
            ...
            for op_name, op_config in operations.items():
                for action in op_config["actions"]:
                    try:
                        await emit_status(
                            op_config["status_msg"](action),
                            emitter=emitter,
                            status="in_progress",
                        )
                        any_updated = True
                    except Exception as e:
                        self.log(f"Failed to emit status: {e}", level="error")
            if any_updated:
                await emit_status("Memory changed", emitter=emitter, status="complete")
    ...
    def inlet(
        ...
        if "messages" in body:
            system_message = next(
                (message for message in body["messages"] if message.get("role") == "system"),
                None,
            )
            if system_message:
                ...
                body["messages"].insert(
                    0, {"role": "system", "content": AUTO_MEMORY_SYSTEM_HINT}
                )
